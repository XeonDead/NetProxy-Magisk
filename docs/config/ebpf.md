...
`EBPF_BYPASS_RULE_SETS="direct,ChinaIP"` 会从可用规则集中提取纯 IP CIDR，并在 eBPF 层直接放行。多个规则集使用英文逗号分隔。被提前绕过的连接不会进入 sing-box 路由；如需严格 Global 模式，请将该值设为空。
...
| `APP_ANDROID_USERS` | 空 | 生效的 Android 用户 ID，多个值使用英文逗号分隔；留空表示全部用户 |
| `PROXY_APPS_LIST` | 空 | 白名单包名，多个值使用英文逗号分隔 |
| `BYPASS_APPS_LIST` | 空 | 黑名单包名，多个值使用英文逗号分隔 |
...
| `EBPF_SHARED_INTERFACES` | `wlan2` | 下游接口名，多个值使用英文逗号分隔 |
| `EBPF_SHARED_INCLUDE_SOURCE_CIDRS` | 空 | 只接管指定来源网段，多个值使用英文逗号分隔 |
| `EBPF_SHARED_EXCLUDE_SOURCE_CIDRS` | 空 | 绕过指定来源网段，多个值使用英文逗号分隔 |
| `EBPF_SHARED_INCLUDE_MAC_ADDRESSES` | 空 | 只接管指定来源 MAC 地址，多个值使用英文逗号分隔 |
| `EBPF_SHARED_EXCLUDE_MAC_ADDRESSES` | 空 | 绕过指定来源 MAC 地址，多个值使用英文逗号分隔 |
