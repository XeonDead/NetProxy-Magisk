...
#######################################
# Inspection of service operating environment
#######################################
verify_environment() {
  ...
  require_file "$TPROXY_CONF_DIR/tproxy.conf" "A transparent proxy configuration file does not exist: $TPROXY_CONF_DIR/tproxy.conf"
  require_dir "$SINGBOX_DIR" "sing-box Configuration directory does not exist: $SINGBOX_DIR"
  require_dir "$CONFDIR" "Universal Configuration Directory does not exist: $CONFDIR"

  ensure_dir "$MODDIR/logs" "Could not create log directory: $MODDIR/logs"
  ensure_dir "$RUNTIME_DIR" "Could not create running-time directory: $RUNTIME_DIR"
}

#######################################
# Clear run-time files
#######################################
...
do_start() {
  ...
  log "INFO" "========== Start start sing-box Services =========="
  verify_environment
  ...
  [ "$RUNTIME_NODE_COUNT" -gt 0 ] || die "The current node directory does not have no node configuration to load: $CUR_OUTBOUND_DIR"
  ...
  log "INFO" "Loaded Node: $RUNTIME_NODE_COUNT, skip invalid node: $RUNTIME_SKIPPED_COUNT"
...
EOF
  ...
  cd "$SINGBOX_DIR" || die "Could not get into configuration directory: $SINGBOX_DIR"
  ...
  if kill -0 "$new_pid" 2> /dev/null; then
    log "INFO" "sing-box Startup successful. (PID: $new_pid)"
  else
    die "sing-box Failed to start, check log: $SINGBOX_LOG_FILE"
  fi
  ...
  if api_wait_available 5 1; then
    ...
    log "WARN" "The control interface is not ready, no running mode is synchronized"
  fi

  log "INFO" "Loading transparency proxy rules..."
  sh "$TPROXY_SCRIPT" start -d "$TPROXY_CONF_DIR" >> "$LOG_FILE" 2>&1 || die "Failed to load transparent proxy rules"
...
do_stop() {
  ...
  log "INFO" "Clearing transparent proxy rules..."
  sh "$TPROXY_SCRIPT" stop -d "$TPROXY_CONF_DIR" >> "$LOG_FILE" 2>&1 || true

  pid="$(get_pid "$SING_BOX_BIN")"
  if [ -z "$pid" ]; then
    log "INFO" "Not found running sing-box Process"
    cleanup_runtime_files
  ...
  if kill "$pid" 2> /dev/null; then
    ...
    if kill -0 "$pid" 2> /dev/null; then
      log "WARN" "Process not responded SIGTERMChange it. SIGKILL"
...
#######################################
do_restart() {
  log "INFO" "========== Start restarting sing-box Services =========="
  do_stop
  sleep 1
  do_start
}
